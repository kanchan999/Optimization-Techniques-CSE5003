# Optimization Techniques — CSE(DS)-5003

This repository contains open course materials for 
**Optimization Techniques (CSE(DS)-5003)** at the 
Indian Institute of Information Technology, Bhopal.

**Dr. Kanchan Rajwar**
Department of Mathematics, IIIT Bhopal

---


## Open Notebook in Colab

Click the badge below to open the Python notebook 
directly in Google Colab. No installation required.

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/kanchan999/Optimization-Techniques-CSE5003/blob/main/code/GA_Introduction.ipynb)

---

## How to Use

1. Read the class notes (`notes/` folder)
2. Work through the Python notebook (`code/` folder)
3. Attempt the question bank (`questions/` folder) for self-assessment
4. Check your answers against the answer key at the end of the question bank

---

## Licence

This material is licensed under 
[CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/) — 
free to use for educational purposes with attribution. 
Not for commercial use.
---

Dr. Kanchan Rajwar
Department of Mathematics
Indian Institute of Information Technology, Bhopal
kanchan.rajwar@iiitbhopal.ac.in
